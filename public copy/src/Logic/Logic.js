...
class Logic {
  ...
  landmarksCompress(originalFloat32Array) {
    const compressedArray = pako.gzip(
      new Uint8Array(originalFloat32Array.buffer)
    );
  ...
  unzip(commpresedData) {
    if (
      commpresedData.startsWith(this.prefix) &&
      commpresedData.endsWith(this.suffix)
    ) {
      commpresedData = commpresedData.substring(
        this.prefix.length,
        commpresedData.length - this.suffix.length
      );
      // var jsonString = atob(commpresedData); //from base64

      var jsonString = Buffer.from(commpresedData, "base64").toString(); //from base64

      const json = JSON.parse(jsonString); //from base64
  ...
  landmarksDeCompress(photoDescriptor) {
    const decodedString =
      // Buffer.from(photoDescriptor, "base64").toString(); //from base64
    ...
    const decompressedArray = pako.ungzip(decodedArray);
